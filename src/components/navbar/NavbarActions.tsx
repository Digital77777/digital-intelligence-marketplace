...
const NavbarActions = () => {
  ...
  return <>
      <div className="flex items-center gap-2 md:gap-4">
        ...
        {/* Search Button */}
        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-white hover:bg-white/20" onClick={openSearch} title="Quick Search (Press / or Ctrl+K)">
          <Search className="h-4 w-4" />
        </Button>
        
        {/* Notification Button */}
        <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-white hover:bg-white/20 relative" onClick={handleNotificationClick} title="Notifications">
