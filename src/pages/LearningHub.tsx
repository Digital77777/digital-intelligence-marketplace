  ...
  return <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 pt-24 px-4 md:px-6 pb-12 bg-gradient-to-b from-indigo-50/30 to-white dark:from-indigo-950/10 dark:to-gray-950">
        <div className="max-w-7xl mx-auto">
          ...
          <LearningHubFilters searchQuery={searchQuery} setSearchQuery={setSearchQuery} setCategoryFilter={setCategoryFilter} setDifficultyFilter={setDifficultyFilter} />
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-8">
            ...
            <TabsContent value="courses" className="mt-6">
              <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                ...
                <Button variant="ghost" className="flex items-center gap-2 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 dark:hover:bg-amber-700" onClick={() => setShowAIAssistant(true)}>
                  <Sparkles className="h-5 w-5 text-amber-500" /> Ask the AI
                </Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading && [...Array(6)].map((_, index) => <Card key={index} className="overflow-hidden">
                ...
                {courses.map(course => <CourseCard key={course.id} course={course} progress={userProgress[course.id] || 0} isCompleted={completedCourses.has(course.id)} onMarkComplete={() => markCourseComplete(course.id)} />)}
              </div>
            </TabsContent>
            ...
            <TabsContent value="paths" className="mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {learningPaths.length === 0 && !isLoading && <div className="text-center text-muted-foreground col-span-full py-8">
                      No learning paths found for your tier.
                    </div>}
                  {learningPaths.map(path => <Card key={path.id} className="overflow-hidden hover:shadow-lg transition-all duration-300">
                      <CardHeader>
                      ...
                      <CardContent>
                        <ul className="mb-4 list-disc list-inside pl-1 space-y-1">
                          {path.courses.map(course => <li key={course.id}>
            ...
            <TabsContent value="events" className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {liveEvents.map(event => <Card key={event.id} className="overflow-hidden">
                    <div className="relative h-32 bg-gradient-to-r from-green-500 to-blue-600 flex items-center justify-center">
                      <Video className="h-8 w-8 text-white" />
                      {isContentLocked(event.required_tier) && <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                    ...
                    <CardFooter>
                      <Button className="w-full" onClick={() => handleJoinEvent(event.id, event.required_tier)} disabled={isContentLocked(event.required_tier)}>
            ...
            <TabsContent value="certifications" className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {certifications.map(cert => <Card key={cert.id} className="overflow-hidden">
                     <div className="relative h-32 bg-gradient-to-r from-yellow-500 to-orange-600 flex items-center justify-center">
                       <Award className="h-8 w-8 text-white" />
                        {isContentLocked(cert.required_tier) && <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <Lock className="h-5 w-5 text-white" />
                          </div>}
                    </div>
                    <CardContent className="pt-4">
                    ...
                    <CardFooter>
                      <Button className="w-full" onClick={() => handleStartCertification(cert.id, cert.required_tier)} disabled={isContentLocked(cert.required_tier)}>
            ...
            <TabsContent value="youtube" className="mt-6">
              <YouTubeCourses searchQuery={searchQuery} category={categoryFilter} difficulty={difficultyFilter} />
            </TabsContent>
          </Tabs>
          ...
          {currentTier !== 'pro' && <Card className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/20 dark:to-indigo-950/20 border-purple-100 dark:border-purple-900/30 p-6 mt-8">
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <Button className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white min-w-[150px]" onClick={() => upgradePrompt(currentTier === 'basic' ? 'pro' : 'basic')}>
